...
public class EffectsEventManager : EventedEventManager {
    ...
    [GameEvent(true, false)]
    public IEnumerator FadeOut(string name, Vector3 movement, Vector3 sizeChange, float duration = 0.5f, GoEaseType type = GoEaseType.ElasticOut)
    {
        var go = GameObject.Find(name);
        if (!go)
        {
    ...
    [GameEvent(true, false)]
    public IEnumerator FadeIn(string name, Vector3 movement, Vector3 sizeChange, float duration = 0.5f, GoEaseType type = GoEaseType.ElasticIn)
    {
        var go = GameObject.Find(name);
        if (!go)
        {
